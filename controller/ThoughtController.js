...
module.exports = class ThoughtController {
  static async register(req,res) {
    ...
    //validation
    if (!image) {
      return res.status(422).json({message: 'Choose file is required!'})
    }
    if (!title || !description || !code) {
      return res.status(422).json({message: 'Title and description and code are required!'})
    }
  ...
  static async update(req, res) {
    ...
    const thought = {title, description, image}
    if (!image) {
      return res.status(422).json({message: 'Choose file is required!'})
    }
  ...
  static async upload(req, res) {  
    ...
    res.json({message: 'Upload make with success!'})
  }
}